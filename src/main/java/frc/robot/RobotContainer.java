...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        joystick2.leftBumper().whileTrue(mouth.run(() -> mouth.setPosition(-0.25)))// mouth.setLeft(drivetrain.getState().Pose)))//
                .whileFalse(mouth.runOnce(() -> mouth.stop())); // 收起

        joystick2.rightBumper().whileTrue(mouth.run(() -> mouth.setPosition(0.2)))// mouth.setRight(drivetrain.getState().Pose)))
                .whileFalse(mouth.runOnce(() -> mouth.stop()));// 展开

        joystick2.axisLessThan(4, -0.9).whileTrue(mouth.run(() -> mouth.setLeft(drivetrain.getState().Pose)));
        // joystick2.axisGreaterThan(4, 0.9).whileTrue(mouth.run(() -> mouth.setLeft(drivetrain.getState().Pose)));
    }
